...
6. **Merged spell availability derivation**: cross-references each merged spell's `requires[]` against `selectedSpells`. A merged spell becomes available when every ID in `requires[]` is present in `selectedSpells`. Feeds `ctx.grantedSpells` at Stage 0 alongside active abilities' `grantsSpells` contributions.
...
19. **Memory-capacity slot wiring end-to-end** — `slots: { type: "spell", count: N }` on Spell Memory / Music Memory skills contributes to `memoryCapacity`. Validator accepts the shape; runtime must aggregate these into the memory budget and the UI must enforce it against `selectedSpells` total `memoryCost`.
...
21. **`form.attacks[i].appliesStatus` walker** — Druid Panther Scratch bleed, Rat Infected Fangs plague, Penguin Sharp Beak bleed. Validator currently only walks wildSkill.effects. Add form.attacks[i].appliesStatus to validator traversal.
    ...
    **Form-attack `id:` convention (follow-up, not a D.21 blocker):** `form.attacks[i]` entries currently have `name:` but no `id:`. Addressing by position within the parent form is fine for the walker work above and for current UI display (name is unique within a form). Assign stable `id:` fields the first time any of the following land: (a) an `abilityModifier` with `target.id` pointing at a specific form attack, (b) a `condition.type: "effect_active"` referencing a specific form attack by effectId, (c) shareable-build URL serialization of "currently simulating attack X", (d) `disables[]` filter pointing at a single form attack. Authoring convention going forward: **any ability-like node that might be referenced externally gets an `id:`.** Tracked here until triggered.
22. **`form.attacks[i].frenziedEffect`** — Druid Panther Neckbite silence triggered only while frenzy_active. Validator must walk frenziedEffect.appliesStatus / frenziedEffect.damage / frenziedEffect.effects, gated by `frenzy_active` condition at runtime. (See D.21 note on form-attack `id:` convention.)
...
27. **Recipe-ID / STAT_META namespace collision (`pdr`/`mdr`)** — ✅ RESOLVED 2026-04-14. Two namespaces are now explicit:
    - **STAT_META keys** — gear/perk *additive contributions*. The old `physicalDamageReduction` / `magicalDamageReduction` additive keys were renamed to `physicalDamageReductionBonus` / `magicalDamageReductionBonus` to remove any read ambiguity with the recipe outputs. All gear-bonus class-data effects updated accordingly.
    - **Recipe IDs** — computed stat outputs, exported as `RECIPE_IDS` from `src/engine/recipes.js` (derived programmatically from `DERIVED_STAT_RECIPES` so it can't drift). `pdr`/`mdr` remain the targets of `phase: "cap_override"` effects at `fighter.js:34` (Defense Mastery) and `barbarian.js:122` (Iron Will).

    The `defineClass` validator now enforces the split: `effect.stat` must resolve to either (a) a STAT_META key / CORE_ATTR / the `all_attributes` sentinel, or (b) a `RECIPE_IDS` entry *and* `phase === "cap_override"`. Anything else — including a recipe ID under the wrong phase, or the old additive name — errors at class-load time.

    Authoring rule:
    - `+5% PDR` from gear/perk → `{ stat: "physicalDamageReductionBonus", phase: "post_curve", value: 0.05 }`
    - Raise the PDR cap → `{ stat: "pdr", phase: "cap_override", value: 0.75 }`

    Flag (not addressed here): `hp`/`ppb`/`mpb` short recipe IDs pre-date this pass and do not currently collide with STAT_META keys — no action, but worth revisiting if any future perk targets them via `cap_override`.
...
- **`grantsSpells` auto-display rule:** when an ability with `grantsSpells: [...]` is active, the granted spells' effects must auto-appear in the snapshot without a separate toggle. Rationale: in-game, the granter ability makes the granted spells' effects reachable via its own mechanism (e.g. Druid Orb of Nature delivers Nature's Touch on ally hit; Warlock Blood Pact makes Bolt of Darkness bare-handed castable). User selecting the granter implies both branches are reachable and should be visualized. No data-shape change required — `grantsSpells` already correctly captures the availability relation; this is an engine effect-collection + UI display rule only. Anchors: `druid.orb_of_nature` grants `natures_touch`; `warlock.blood_pact` grants `bolt_of_darkness`. Explicit non-goal: do NOT inline granted-spell effects into granter authoring; single-source-of-truth stays on the granted spell's own definition.
