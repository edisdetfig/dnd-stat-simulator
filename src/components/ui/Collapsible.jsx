...
export function Collapsible({ title, color, defaultOpen, badge, badgeBg, children }) {
  ...
  return (
    <div>
      <div onClick={() => setOpen(!open)} style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer", padding: "5px 0", userSelect: "none" }}>
        ...
        {badge != null && <span style={{ fontSize: 9, background: badgeBg || "transparent", color: color || "var(--sim-text-whisper)", padding: "1px 6px", borderRadius: 10, marginLeft: "auto" }}>{badge}</span>}
      </div>
